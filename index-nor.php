<?php include("header-nor.php") ?>
		<!-- content holder -->	
		<div class="content cf">
			<!-- primary-content holder -->
			<div class="primary-content"> 
				<!-- youtube link-->
				<br><a class="video"  title="Dare to be Digital - apply now" href="http://youtube.googleapis.com/v/7r63s5dVTlA?rel=0;autoplay=1"><img src="img/dareapplynowvideo.png" alt="youtubepicture" /></a>
				<!-- text holder, containing a middle boarder divinding text and pictures in two sections -->    
				<div class="textholder cf main">
					<div class="textsection">	
						<p>Liker du å spille videospill? Ser du seriøs angående designe videospill? Hvis dette stemmer, har du kommet til rett plass! Dare to be Digital er det fremste videospill-utvikler konkurransen for  studenter på universitet og høgskoler. Dare to be Digital Nordisk er plassen hvor du vil finne all informasjon som en trenger for å delta i konkuransen</p>
					</div>
					<div class="illustration">
						<a href="http://www.daretobedigital.com/d2bd/login-or-register/login.php?accesscheck=apply-2013"><img src="img/applynowsmall.jpg" alt="apply now"></a>
					</div>
				</div><!-- end of text holder -->	
				<h2>Nyheter</h2>
					<!-- news section outer boarder -->
					<div class="news cf">
						<!-- news article and inner boarder -->
						<div class="news-section cf">
							<img src="img/440_1thumb.jpg" />
								<strong>7. Mars 2013</strong>
									<p>Det finske og skotske teamet bak Starcrossed, en av Dare to be Digital vinnende spillene fra 2012, vant en BAFTA “Ones to Watch”-prisen  i går kveld på British Academy Games Awards. Andrew MacLean sa i går kveld: “Å vinne BAFTA har ikke sunket ordentlig inn enda. Ord kan helt ærlig ikke beskrive følelsen. Da jeg sto på scenen, trodde jeg at jeg skulle falle for beina mine ristet så fælt.”</p>
						</div><!-- end of news article -->
						<!-- news article and inner boarder -->
						<div class="news-section cf">
							<img src="img/439_2thumb.jpg" />
								<strong>4. Mars 2013</strong>
								<p>BAFTA-vinnende student spillet Tick Tock Toys er allerede en global hit for dens skapermottakende salgsfremmende funksjoner på Apples App Store i 40 land verden over. Sophia George, co-grunnlegger av Swallowtail Games, sa: “Vi er absolutt henrykt over å få promotert vårt første spill, Tick Tock Toys på Apples App Store. Det er en utrolig utvikling etter å ha vunnet Dare 2011 og BAFTA “Ones To Watch” –prisen i 2012.”</p>
						</div><!-- end of news article -->
						<!-- news article and inner boarder -->
						<div class="news-section cf">
							<img src="img/438_2thumb.jpg" />
								<strong>23. Januar 2013 </strong>
									<p>Dare to be Digital, verdens ledende dataspill og design konkurranse for studenter er åpen for påmelding! En unik mulighet for ambisiøse fagfolk til å vinne deres egen BAFTA. Drevet av University of Abertay Dundee, ser Dare to be Digital hundrevis av studenter hvert år konkurrere om å bli en av de 15 lagene til å lage en fungerende spill prototype på bare 9 uker. Deretter går spillene til et show for publikum og bransjeeksperter i Dundee i august med tre vinnende lag valgt til å konkurrere om BAFTA “Ones to Watch”-prisen.</p>
						</div><!-- end of news article -->
					</div><!-- end of news section -->
					<br/>
					<!-- text holder, containing a middle boarder divinding text and pictures in two sections -->
					<div class="textholder cf main">
						<div class="textsection">	
							<p>Studenter eller nylig bachelor studenter fra de Nordiske landende vil konkurrere mot hverandre i en Nordisk del av Dare to be Digital konkurransen. Lagene må legge ved en liten video snutt av deres spill konsept, når de melder seg på den Nordiske konkurransen. Video ideen vil bli vurdert av dommerne på nett, og vil plukke ut noen lag som vil bli innkalt til en intervju dag som vil ta plass på NITH i Oslo, Norge.</p>
						</div>
						<div class="illustration2">
							<p>Vinneren av den Nordiske konkurransen vil bli offentligjort under kåringen av den Nordiske vinneren som vil finne sted den 15. mai, på NITH i Oslo, Norge. Så hva er prisen til den Nordiske vinneren av konkurransen? Den Nordiske vinneren vil få alle reisekostnader dekket for å reise til Dundee, Skottland. I tillegg til dette vil laget også få en ukentlig lønn gjennom alle de 9 ukene de er i Skottland for å utvikle spille konseptet sitt.</p>
						</div>
					</div><!-- end of text holder -->	
						<a href="http://www.daretobedigital.com/d2bd/login-or-register/login.php?accesscheck=apply-2013"><img src="img/applynowsmall.jpg" alt="apply now"></a>
			</div><!-- end of primary content -->
			<!-- Sidebar content holder -->
			<div class="sidebar">
				<div class="events">
					<h2>Viktige datoer</h2>
						<table>
							<tr>
								<td>Søknadsfrist</td>
								<td>9 Apr</td>
							</tr>
							<tr>
								<td>Online dømming</td>
								<td>15 Apr</td>
							</tr>
							<tr>
								<td>Intervjuer</td>
								<td>29 Apr - 9 May</td>
							</tr>
							<tr>
								<td>Kåring av Nordisk vinner på NITH</td><!-- byttet ut "it req" skal denne tas med? -->
								<td>15 May</td>
							</tr>
							<tr>
								<td>Konkurranse datoer</td>
								<td>10 Jun - 11 Aug</td>
							</tr>
							<tr>
								<td>Dare ProtoPlay</td>
								<td>8 - 11 Aug</td>
							</tr>
						</table>
				</div><!-- events end -->
				<!-- Facebook stream box -->
				<iframe src="//www.facebook.com/plugins/likebox.php?href=http%3A%2F%2Fwww.facebook.com%2Fdarenordic&amp;width=320&amp;height=558&amp;show_faces=true&amp;colorscheme=light&amp;stream=true&amp;border_color&amp;header=false" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:320px; height:558px;" allowTransparency="true"></iframe>				
				<div class ="nith_add">
					<a href="http://nith.no"><img src="img/nithbutton.jpg" alt="NITH add"></a>
				</div>
				<div class="dareinternational_add">
					<a href="http://www.daretobedigital.com/index.php"><img src="img/dareint.jpg" alt="Dare international add"></a>
				</div>	
			</div><!-- sidebar end -->	
		</div><!-- content end -->
	</div><!-- wrapper end -->
	<!-- footer wraper -->
	<div class="footer">
		<center>All images, videos, content Copyright © 2013 Dare to be Digital Limited except where noted. All Rights Reserved. <a href="mailto:enquiries@daretobedigital.com">enquiries@daretobedigital.com</a><br />Website Design by MTC Media | Dare 2011 illustrations by Sooper Double D</center>
	</div><!-- footer end -->
</body>
</html>